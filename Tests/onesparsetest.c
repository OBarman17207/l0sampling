...
int one_sparse_test(int sprime) {
  struct one_sparse* one_sparse_recovery = malloc(sizeof(struct one_sparse));
  one_sparse_recovery->sum_of_weights = 0;
  one_sparse_recovery->sum_of_identifiers = 0;
  one_sparse_recovery->sum_of_fingerprints = 0;
  if(sprime > 0){
    one_sparse_recovery->zeta = rand() % sprime;
  }
  int array_size = rand() % 10000;
  //int array_size = sprime;
  int random_index = (rand() % (array_size - 1) + 1);
  //printf("random index is %d\n",random_index);
  for(int i = 0; i < array_size; i ++){
    if( i == random_index){
      update_one_sparse_recovery(one_sparse_recovery, i, rand(), sprime);
    }
    else{
      update_one_sparse_recovery(one_sparse_recovery, i, 0, sprime);
    }
  }
  struct one_sparse* final_vector = recover_one_sparse(one_sparse_recovery,sprime);
  int result;
  if(final_vector){
